...
(async () => {
  ...
  await page.click(
    "#aspnetForm > div.page > div.header > div.hideSkiplink > div > ul:nth-child(4) > li > a"
  );
  ...
  for (i = 0; i < DAYS_AGO; i++) {
    ...
    csvStream.write({
      ...
      VefatSayisi2020: count2020,
      VefatSayisi2019: count2019,
      VefatSayisi2018: count2018,
    });
...
async function changeDateAndSearch(page, dateStr, debug = false) {
  ...
  return Promise.all([
    ...
    page.waitFor(
      ...
      {},
      dateStr
    ), // wait until DOM changes to new date. Pass dateStr to browser context as locDateStr.
  ]);
}
...
async function countElements(page) {
  return page.$eval(
    "#ctl00_MainContent_UpdatePanel2 > div:nth-child(1)",
    (elem) => {
...
async function scrollToTopAndScreenshot(page, dateStr, cityName) {
  ...
  await page.screenshot({
    path: `./screenshots/${cityName}/${dateStr}.png`,
    fullPage: true,
  });
